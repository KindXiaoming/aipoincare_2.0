...
// Threebody: 5 (variable) * 3 (sum num)
// KdV: 2 (variable) * 40 (sum num)
// Quantum: 2 (variable) * 40 (sum num)
...
T H_evaluate_(vector<char> RPN, int arity_len, vector<T> z){
    vector<T> stacks;
    int j = 0;
...
// Num Arity Simplify
T H_evaluate(vector<char> RPN, int arity_len, vector<T> z, int variable_num, int piece_num){
    T temp = 0.;
    for (int i=0;i<piece_num;i++){
        vector<T> z_piece = std::vector<T>(z.begin()+i*variable_num, z.begin()+(i+1)*variable_num);
        //cout << endl;
        //printvec(z_piece);
        //cout << endl;
        T add = H_evaluate_(RPN, arity_len, z_piece);
        //cout << endl << add << endl;
        temp = temp + add;
    }
    return temp;
...
vector<T> H_grad_evaluate(vector<char> RPN, int arity_len, vector<T> z, const int dim, const int num_pieces){
    T eps = 1e-6;
    vector<T> grads;
    for (int i=0;i<dim*num_pieces;i++){
        vector<T> zp = z;
        vector<T> zm = z;
        zp[i] = zp[i] + eps;
        zm[i] = zm[i] - eps;
        T Hp = H_evaluate(RPN, arity_len, zp, dim, num_pieces);
        T Hm = H_evaluate(RPN, arity_len, zm, dim, num_pieces);
        T grad = (Hp-Hm)/(2*eps);
...
int main(){
    //[env, input_dim, num_pieces]
    //[threebody_2d, 5, 3]
    //[KdV_1d, 2, 40]
    //[quantum, 2, 40]
    //string env = "threebody_2d";
    string env = "KdV_1d";
    const int input_dim = 80;
    const int variable_dim = 2;
    const int num_pieces = 40;
    int test_pts = 1;
    char variables2[] = {'a','b','c','d','e','f','\0'};
    char variables[variable_dim];
    if (variable_dim > 6){cout << "Support <7 variables!";};
    for (int i=0;i<variable_dim;i++){
        variables[i] = variables2[i];
    }
    variables[variable_dim] = '\0';
    ...
    int num_templates = 216;//89(7),216(8),539(9);
    int template_id = 0;
    ...
    for (int k=0;k<num_templates;k++){
        ...
        for (int i=0; i<flat_size;i++){
            ...
            for (int j=0; j<test_pts; j++){
                vector<T> grad = H_grad_evaluate(RPN, arity_len, data_var[j], variable_dim, num_pieces);
                ...
                //printrpn(RPN);
                //cout << endl << eps << endl;
                if (eps>0.0001 || isnan(eps)){flag=false; break;}
                }
            if (flag==true){
                //printrpn(RPN);
                //cout << endl << eps << endl;
